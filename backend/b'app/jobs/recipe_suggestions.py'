...
def computeRecipeSuggestions(household_id: int):
    historyCount = RecipeHistory.query.with_entities(RecipeHistory.recipe_id, func.count().label('count')).filter(
        RecipeHistory.status == Status.ADDED, 
        RecipeHistory.household_id == household_id,
        RecipeHistory.created_at >= datetime.datetime.utcnow() - datetime.timedelta(days=182),
        RecipeHistory.created_at <= datetime.datetime.utcnow() - datetime.timedelta(days=7),
    ).group_by(RecipeHistory.recipe_id).all()
