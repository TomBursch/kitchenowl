...
def test_list_users_user(user_client):
    response = user_client.get('/api/user/all',)
    assert response.status_code == 403


def test_list_users_admin(admin_client, admin_username, admin_name, username, name, password):
    response = admin_client.get('/api/user/all',)
    assert response.status_code == 200
    data = response.get_json()
    assert len(data) == 1
