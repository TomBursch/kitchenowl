...
| `JWT_SECRET_KEY`                           | _(required)_               | Secret with at least 32 bytes used to sign tokens. Known defaults are rejected outside debug mode.                                                    |
| `JWT_SECRET_KEY_FILE`                      |                            | Reads the JWT secret from the specified file. Should be a path to a file (will override JWT_SECRET_KEY)                                               |
| `LLM_ENCRYPTION_KEY`                       | _(required outside debug)_ | Fernet key used to encrypt recipe-agent provider credentials.                                                                                         |
| `LLM_ENCRYPTION_KEY_PREVIOUS`              |                            | Comma-separated previous Fernet keys accepted during credential-key rotation.                                                                         |
| `LLM_ALLOWED_HOSTS`                        |                            | Comma-separated LLM endpoint hosts. Private and local targets must be listed explicitly.                                                               |
| `TRUSTED_PROXIES`                          |                            | Comma-separated proxy IPs or CIDRs allowed to supply `X-Forwarded-For`.                                                                                |
...
| `TERMS_URL`                                |                            | Allows to set a custom terms and conditions URL for your server instance                                                                              |
| `DISABLE_USERNAME_PASSWORD_LOGIN`          | `false`                    | If set, allows login only through OpenID Connect (OIDC). Be aware: this automatically disables `OPEN_REGISTRATION`                                    |
...
| `DB_SQLITE_SYNCHRONOUS`                    | `FULL`                     | SQLite only. `synchronous` PRAGMA. `NORMAL` is far faster on slow disks (e.g. NAS); `FULL` (default) is safest. One of `OFF`/`NORMAL`/`FULL`/`EXTRA`  |
| `DB_USER`                                  |                            |                                                                                                                                                       |
