...
| `DISABLE_FUZZY_SEARCH`                     | `false`                    | If set, disables typo-tolerant item search and falls back to prefix/substring match. Recommended for instances with large item catalogs where the fuzzy search becomes a bottleneck. |
